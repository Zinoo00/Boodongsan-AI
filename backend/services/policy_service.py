...
class PolicyService:
    ...
    async def find_applicable_policies(
        ...
        try:
            async with get_db_session() as db:
                # 활성 정책만 조회
                base_query = db.query(GovernmentPolicy).filter(
                ...
                if user_profile.get("annual_income"):
                    ...
                    income_filter = and_(
                        or_(GovernmentPolicy.income_min.is_(None), GovernmentPolicy.income_min <= income),
                        or_(GovernmentPolicy.income_max.is_(None), GovernmentPolicy.income_max >= income)
                    )
        ...
        try:
            async with get_db_session() as db:
                ...
                conditions = db.query(PolicyCondition).filter(
                    PolicyCondition.policy_id == policy_id
                ).all()
    ...
    async def calculate_policy_benefit(
        ...
        try:
            ...
            if available_loan > 0 and interest_rate > 0:
                ...
                # 원리금균등상환 공식
                monthly_payment = available_loan * (
            ...
            if available_loan > 0:
                ...
                market_monthly_payment = available_loan * (
        ...
        try:
            ...
            if user_age:
                ...
                if age_min and user_age < age_min:
                    results["failed_conditions"].append(f"최소 나이 {age_min}세 이상 필요 (현재: {user_age}세)")
                    results["is_eligible"] = False
                elif age_max and user_age > age_max:
                    results["failed_conditions"].append(f"최대 나이 {age_max}세 이하 필요 (현재: {user_age}세)")
                    results["is_eligible"] = False
                else:
    ...
    async def search_policies_by_keyword(
        ...
        try:
            async with get_db_session() as db:
                policies = db.query(GovernmentPolicy).filter(
                    and_(
                        GovernmentPolicy.is_active == True,
                        or_(
