...
class RAGService:
    ...
    async def process_query(
        ...
        try:
            ...
            await self._save_conversation_history(context, user_id, conversation_id, ai_response)
    ...
    async def _load_user_context(self, context: RAGContext, user_id: str, conversation_id: str):
    ...
    async def _extract_entities_and_intent(self, context: RAGContext):
        ...
        try:
            # Extract entities
            context.extracted_entities = await self.ai_service.extract_entities(context.user_query)

            # Classify intent
            context.intent = await self.ai_service.classify_intent(context.user_query)
    ...
    async def _perform_vector_search(self, context: RAGContext):
        ...
        try:
            ...
            query_embedding = await self.vector_service.generate_embedding(
                context.user_query, correlation_id=context.correlation_id
            )
    ...
    async def _retrieve_relevant_properties(self, context: RAGContext):
        ...
        try:
            ...
            properties = await self.property_service.search_properties(
                filter=property_filter, limit=self.max_search_results
            )
            ...
            context.relevant_properties.sort(key=lambda x: x.relevance_score, reverse=True)
        ...
        try:
            if context.user_profile:
                ...
                context.relevant_policies = await self.policy_service.search_policies_by_query(
                    query=context.user_query, limit=3
                )
    ...
    async def _generate_ai_response(self, context: RAGContext) -> dict[str, Any]:
        ...
        try:
            ...
            response = await self.ai_service.generate_rag_response(
                context=ai_context, max_tokens=settings.RESPONSE_MAX_TOKENS
            )
    ...
    async def _save_conversation_history(
        self, context: RAGContext, user_id: str, conversation_id: str, ai_response: dict[str, Any]
    ):
        ...
        try:
            ...
            await self.user_service.save_conversation_message(
                ...
                recommended_policies=[
                    str(policy.policy.id) for policy in context.relevant_policies[:3]
                ],
            )
    ...
    def _calculate_property_relevance(self, property_obj: Any, context: RAGContext) -> float:
    ...
    def _identify_matching_criteria(self, property_obj: Any, context: RAGContext) -> list[str]:
        ...
        if (
            "transaction_type" in entities
            and entities["transaction_type"] == property_obj.transaction_type
        ):
    ...
    async def _cache_response(
        self, validated_input: QueryValidation, response: dict[str, Any], correlation_id: str
    ):
        ...
        try:
            ...
            logger.warning(
                f"Failed to cache response: {str(e)}", extra={"correlation_id": correlation_id}
            )

    def _compile_response(
        self, context: RAGContext, ai_response: dict[str, Any], pipeline_start: float
    ) -> dict[str, Any]:
    ...
    def _create_validation_error_response(
        self, start_time: float, error_msg: str
    ) -> dict[str, Any]:
