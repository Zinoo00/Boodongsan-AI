...
class VectorService:
    ...
    async def initialize(self):
        ...
        try:
            logger.info(
                "Initializing Qdrant vector service",
    ...
    async def _generate_embedding_impl(
        ...
        try:
            ...
            # - AWS Bedrock embeddings: bedrock_runtime.invoke_model()
            # - Sentence transformers: model.encode()
            # - Cohere embeddings: co.embed()
            
            # Create a deterministic but pseudo-random embedding for testing
        ...
        try:
            ...
            for i in range(0, total_docs, batch_size):
                ...
                if points:
                    ...
                    logger.info(f"Uploaded batch {i//batch_size + 1}/{(total_docs + batch_size - 1)//batch_size} "
    ...
    async def hybrid_search(
        ...
        try:
            ...
            query_tokens = {token for token in query_tokens if len(token) > 1}  # Filter short tokens
    ...
    def _build_filter_conditions(self, filters: dict[str, Any] | None) -> Filter | None:
        ...
        if "area_min" in filters or "area_max" in filters:
            ...
            conditions.append(
                FieldCondition(key="area_m2", range=area_range)
            )
        ...
        if "date_from" in filters or "date_to" in filters:
            ...
            conditions.append(
                FieldCondition(key="created_at", range=date_range)
            )
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            health_status["checks"]["embedding"].update({
                "status": True,
                "latency_ms": round(embedding_latency, 2)
            })
    ...
    async def clear_cache(self, pattern: str = "embedding:*") -> int:
        ...
        try:
            ...
            logger.error(
                "Failed to clear vector service cache",
