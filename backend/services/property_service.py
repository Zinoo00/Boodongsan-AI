...
class PropertyService:
    ...
    async def search_properties(
        self, filter: PropertyFilter, limit: int = 10, offset: int = 0
    ...
    async def search_properties_by_criteria(self, criteria: dict[str, Any]) -> list[dict[str, Any]]:
        """조건 기반 부동산 검색"""
        try:
            # 실제로는 데이터베이스 쿼리 실행
            # 여기서는 임시 더미 데이터 반환
            ...
            for prop in dummy_properties:
                ...
                if (
                    ...
                    and criteria["property_type"] != prop["property_type"]
                ):
                    match = False
