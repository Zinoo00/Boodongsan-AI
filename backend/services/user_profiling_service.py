...
class UserProfilingService:
    ...
    async def create_or_update_profile(
        self, user_id: str, extracted_entities: dict[str, Any], session_id: str = None
    ) -> dict[str, Any]:
        """사용자 프로필 생성 또는 업데이트"""
        try:
    ...
    async def _get_or_create_user(
        self, db: Session, user_id: str, extracted_entities: dict[str, Any]
    ) -> User:
        """사용자 조회 또는 생성"""
        try:
    ...
    async def _update_user_basic_info(
        self, db: Session, user: User, extracted_entities: dict[str, Any]
    ) -> User:
        """사용자 기본 정보 업데이트"""
        try:
    ...
    def _calculate_profile_completeness(
        self, user: User, preference: UserPreference | None
    ) -> float:
        """프로필 완성도 계산 (0.0 ~ 1.0)"""
        try:
            total_fields = 0
    ...
    async def analyze_conversation_patterns(self, user_id: str, limit: int = 10) -> dict[str, Any]:
        """사용자 대화 패턴 분석"""
        try:
            async with get_db_session() as db:
                conversations = (
                ...
                analysis = {
                    ...
                    "last_conversation": conversations[0].created_at.isoformat()
                    if conversations
                    else None,
                }
    ...
    async def suggest_missing_info(self, user_id: str) -> list[str]:
        ...
        try:
            ...
            if not basic_info.get("budget_min") and not basic_info.get("budget_max"):
                suggestions.append(
                    "예산 범위를 알려주시면 조건에 맞는 매물을 추천드릴 수 있습니다."
                )
