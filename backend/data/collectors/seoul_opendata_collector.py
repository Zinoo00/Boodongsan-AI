...
SEOUL_SERVICES: dict[str, SeoulOpenDataService] = {
    ...
    "real_transaction": SeoulOpenDataService(
        service_name="tbLnOpendataRtmsV",  # 실거래가 (매매/전월세 통합)
        description="부동산 실거래가 정보",
        data_code="OA-21275",
        category=DataCategory.REAL_ESTATE,
        update_frequency="daily",
        key_fields=["CGG_NM", "STDG_NM", "RCPT_YR", "ARCH_AREA", "THING_AMT", "ARCH_YR", "BLDG_USG"],
    ),
    # NOTE: 아래 서비스들은 현재 서울시 API에서 ERROR-500 반환 (2024.12 기준)
    # 향후 서비스 재개 시 활성화 가능
    ...
    # ),
    # ========== 정비사업/재개발 데이터 ==========
    # NOTE: 아래 서비스는 ERROR-500 반환 (2024.12 기준)
    ...
    # ),
    "redevelopment": SeoulOpenDataService(
        service_name="upisRebuild",  # 작동 확인됨 (2024.12)
        description="정비사업 현황",
        data_code="OA-20281",
        category=DataCategory.REDEVELOPMENT,
        update_frequency="daily",
    ...
    "subway_station": SeoulOpenDataService(
        service_name="SearchSTNBySubwayLineInfo",  # 작동 확인됨 (2024.12)
    ...
    "subway_info": SeoulOpenDataService(
        service_name="SearchInfoBySubwayNameService",  # 작동 확인됨 (2024.12)
        description="지하철역 정보 (역명으로 조회)",
        data_code="OA-114",
        category=DataCategory.TRANSPORT,
        update_frequency="as_needed",
        key_fields=["STATION_CD", "STATION_NM", "LINE_NUM", "FR_CODE"],
    ...
    "bus_stop": SeoulOpenDataService(
        service_name="busStopLocationXyInfo",  # 작동 확인됨 (2024.12)
        description="버스정류소 위치정보",
        data_code="OA-15067",
        category=DataCategory.TRANSPORT,
        update_frequency="monthly",
        key_fields=["STOPS_NO", "STOPS_NM", "XCRD", "YCRD", "STOPS_TYPE"],
    ),
    # NOTE: 아래 용도지역 서비스들은 현재 서울시 API에서 ERROR-500 반환 (2024.12 기준)
