...
class RealEstateCollector:
    ...
    async def collect_all_data(
        self, year_month: str = None, districts: list[str] = None, property_types: list[str] = None
            ...
            for district in districts:
                ...
                for property_type in property_types:
                    try:
                        ...
                        processed += 1
                        logger.info(
                            f"Completed {processed}/{total_combinations}: {district} - {property_type}"
                        )

                    except Exception as e:
                        logger.error(
                            f"Failed to collect data for {district} - {property_type}: {str(e)}"
                        )
                        continue
    ...
    async def collect_district_data(
        self, district: str, district_code: str, property_type: str, year_month: str
    ) -> AsyncGenerator[dict[str, Any], None]:
        """
    ...
    def _parse_xml_response(
        self, xml_content: str, property_type: str, district: str
    ) -> list[dict[str, Any]]:
    ...
    def _parse_item(
        self, item: ET.Element, property_type: str, district: str
    ) -> dict[str, Any] | None:
        ...
        try:
            ...
            if all(
                field in record
                for field in ["transaction_year", "transaction_month", "transaction_day"]
            ):
                try:
