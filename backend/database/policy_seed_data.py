    ...
    try:
        async with get_db_session() as db:
            ...
            youth_policy = (
                ...
                .filter(GovernmentPolicy.policy_name == "청년전세임대주택")
                .first()
            )
            ...
            if youth_policy:
                ...
                for cond_data in conditions:
                    condition = PolicyCondition(policy_id=youth_policy.id, **cond_data)
                    db.add(condition)

            # 디딤돌 대출 세부 조건
            didimstone_policy = (
                db.query(GovernmentPolicy)
                .filter(GovernmentPolicy.policy_name == "디딤돌 대출")
                .first()
            )
            ...
            if didimstone_policy:
                ...
                for cond_data in conditions:
                    condition = PolicyCondition(policy_id=didimstone_policy.id, **cond_data)
                    db.add(condition)
    ...
    try:
        async with get_db_session() as db:
            ...
            for policy_name, new_rate in policies_to_update:
                policy = (
                    db.query(GovernmentPolicy)
                    .filter(GovernmentPolicy.policy_name == policy_name)
                    .first()
                )
...
async def get_policy_statistics():
    ...
    try:
        async with get_db_session() as db:
            ...
            active_policies = (
                db.query(GovernmentPolicy).filter(GovernmentPolicy.is_active == True).count()
            )

            # 정책 유형별 통계
            policy_types = (
                db.query(GovernmentPolicy.policy_type, db.func.count(GovernmentPolicy.id))
                .group_by(GovernmentPolicy.policy_type)
                .all()
            )

            # 정책 카테고리별 통계
            policy_categories = (
                db.query(GovernmentPolicy.policy_category, db.func.count(GovernmentPolicy.id))
                .group_by(GovernmentPolicy.policy_category)
                .all()
            )
