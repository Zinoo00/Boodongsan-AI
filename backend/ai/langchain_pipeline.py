...
class PolicySearchTool(BaseTool):
    ...
    def _run(self, user_profile: str) -> str:
        ...
        try:
            ...
            results = asyncio.run(
                self.policy_service.find_applicable_policies(profile_dict)
            )
...
class RealEstateAgent:
    ...
    async def _handle_property_search(
        ...
        try:
            ...
            # 부동산 검색
            properties = await self.property_service.search_properties_by_criteria(
                search_criteria
            )
    ...
    async def _handle_policy_inquiry(
        self, 
        message: str, 
        context: ConversationContext
    ) -> str:
        ...
        try:
            ...
            for policy in policies:
                ...
                if policy.age_min or policy.age_max:
                    age_range = f"{policy.age_min or '제한없음'}세 ~ {policy.age_max or '제한없음'}세"
                    info += f"\n- 나이 조건: {age_range}"
