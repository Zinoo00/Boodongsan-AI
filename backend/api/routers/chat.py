    ...
    try:
        ...
        if updated_context.extracted_entities and user_id:
            background_tasks.add_task(
                ...
                user_id,
                updated_context.extracted_entities,
                session_id
            )
...
async def get_conversation_history(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="대화 이력을 불러오는 중 오류가 발생했습니다."
        )
    ...
    try:
        ...
        if cached_context:
            context = ConversationContext(
                user_id=user_id or cached_context.get("user_id"),
                session_id=session_id
            )
...
async def _update_user_profile_async(
    ...
    try:
        profiling_service = UserProfilingService()
        await profiling_service.create_or_update_profile(
            user_id, extracted_entities, session_id
        )
