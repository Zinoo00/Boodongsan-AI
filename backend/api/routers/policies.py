...
async def get_policy_detail(policy_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="정책 상세 정보 조회 중 오류가 발생했습니다.")


@router.post("/{policy_id}/calculate-benefit", response_model=PolicyBenefitResponse)
async def calculate_policy_benefit(policy_id: str, user_profile: dict[str, Any]):
    """사용자 기준 정책 혜택 계산"""
    try:
        policy_service = PolicyService()
        benefit_calculation = await policy_service.calculate_policy_benefit(policy_id, user_profile)

        if "error" in benefit_calculation:
            raise HTTPException(status_code=400, detail=benefit_calculation["error"])
        ...
        raise HTTPException(status_code=500, detail="정책 혜택 계산 중 오류가 발생했습니다.")
...
async def search_policies_by_keyword(
    ...
    try:
        ...
        return PolicyListResponse(
            policies=policies_data,
            total_count=len(policies_data),
            filters_applied={"keyword": keyword, "limit": limit},
        )

    except Exception as e:
        logger.error(f"키워드 정책 검색 실패: {str(e)}")
        raise HTTPException(status_code=500, detail="키워드 정책 검색 중 오류가 발생했습니다.")


@router.get("/popular/list", response_model=PolicyListResponse)
async def get_popular_policies(limit: int = Query(5, description="조회 개수", le=20)):
    """인기 정책 목록 조회"""
    try:
        policy_service = PolicyService()
        policies_data = await policy_service.get_popular_policies(limit)
        ...
        raise HTTPException(status_code=500, detail="인기 정책 조회 중 오류가 발생했습니다.")
...
async def get_policy_types():
    ...
    try:
        # 정적 정책 유형 목록 반환
        policy_types = [
            {"type": "전세자금", "description": "전세 보증금 대출 지원", "count": 3},
            {"type": "구입자금", "description": "주택 구입 자금 대출 지원", "count": 2},
            {"type": "임대주택", "description": "공공 임대주택 공급", "count": 1},
            {"type": "특별공급", "description": "계층별 주택 특별공급", "count": 3},
            {"type": "청약", "description": "주택 청약 관련 지원", "count": 2},
            {"type": "보증보험", "description": "전세 보증금 보험", "count": 1},
        ]

        return {"policy_types": policy_types, "total_types": len(policy_types)}

    except Exception as e:
        logger.error(f"정책 유형 조회 실패: {str(e)}")
        raise HTTPException(status_code=500, detail="정책 유형 조회 중 오류가 발생했습니다.")
...
async def get_policy_categories():
    ...
    try:
        # 정적 정책 카테고리 목록 반환
        policy_categories = [
            {"category": "청년", "description": "만 39세 이하 청년층 대상", "count": 2},
            {"category": "신혼부부", "description": "혼인 7년 이내 신혼부부 대상", "count": 1},
            {"category": "다자녀", "description": "미성년 자녀 3명 이상 가구 대상", "count": 1},
            {"category": "생애최초", "description": "생애최초 주택 구입자 대상", "count": 1},
            {"category": "일반", "description": "일반 계층 대상", "count": 7},
        ]

        return {"policy_categories": policy_categories, "total_categories": len(policy_categories)}

    except Exception as e:
        logger.error(f"정책 카테고리 조회 실패: {str(e)}")
        raise HTTPException(status_code=500, detail="정책 카테고리 조회 중 오류가 발생했습니다.")
