...
class OffiDataCollector(BaseDataCollector):
    """오피스텔 데이터 수집기 (전월세 + 매매)"""

    def make_api_request(self, api_type: str = "offi_rent", lawd_cd: str = None, deal_ymd: str = None) -> str:
        """
        공공데이터포털 API 호출

        Args:
            api_type: API 타입 ("offi_rent", "offi_trade")
        ...
        """
        # API 타입에 따른 URL 선택
        if api_type == "offi_rent":
            base_url = self.config.OFFI_RENT_BASE_URL
            api_name = "오피스텔 전월세"
        elif api_type == "offi_trade":
            base_url = self.config.OFFI_TRADE_BASE_URL
            api_name = "오피스텔 매매 실거래가"
        else:
            raise ValueError("api_type은 'offi_rent' 또는 'offi_trade'여야 합니다.")

        return super().make_api_request(api_type, base_url, api_name, lawd_cd, deal_ymd)

    def parse_response(self, response_text: str, api_type: str = "offi_rent") -> List[Dict]:
        """
    ...
    def _parse_xml_response(self, response_text: str, api_type: str = "offi_rent") -> List[Dict]:
        """XML 응답 파싱"""
        try:
            ...
            for item in items:
                try:
                    if api_type == "offi_rent":
                        # 오피스텔 전월세 데이터 파싱
                        parsed_data = {
                            'office_name': self._get_xml_text(item, 'offiNm'),
                            ...
                            'sgg_name': self._get_xml_text(item, 'sggNm'),
    ...
    def _parse_line(self, line: str, api_type: str = "offi_rent") -> Optional[Dict]:
        """
        ...
        """
        try:
            office_info = line.split('2001')[0] if '2001' in line else line.split('2007')[0] if '2007' in line else line.split('2014')[0] if '2014' in line else line.split('2022')[0] if '2022' in line else line.split('2021')[0] if '2021' in line else line.split('2020')[0] if '2020' in line else line.split('2010')[0] if '2010' in line else line.split('2005')[0] if '2005' in line else line
    ...
    def collect_offi_rent_data(self, lawd_cd: str = None, deal_ymd: str = None) -> dict:
        """
        오피스텔 전월세 데이터 수집 및 DataFrame 반환
        ...
        """
        print("오피스텔 전월세 데이터 수집을 시작합니다...")

        response_text = self.make_api_request("offi_rent", lawd_cd, deal_ymd)
        parsed_data = self.parse_response(response_text, "offi_rent")
        ...
        if isinstance(parsed_data, dict) and 'clean_data' in parsed_data:
            ...
            print(f"오피스텔 전월세 데이터 수집 완료: {len(clean_df)}개 항목 (정제된 데이터)")
            ...
            print(f"오피스텔 전월세 데이터 수집 완료: {len(df)}개 항목")
            return {
                'clean_data': df,
                'raw_data': None,
                'response_text': response_text
    ...
    def collect_offi_trade_data(self, lawd_cd: str = None, deal_ymd: str = None) -> dict:
        """
        오피스텔 매매 데이터 수집 및 DataFrame 반환
        ...
        Returns:
            정제된 데이터와 raw 데이터를 포함한 딕셔너리
        """
        print("오피스텔 매매 실거래가 데이터 수집을 시작합니다...")

        response_text = self.make_api_request("offi_trade", lawd_cd, deal_ymd)
        parsed_data = self.parse_response(response_text, "offi_trade")

        if isinstance(parsed_data, dict) and 'clean_data' in parsed_data:
            clean_df = pd.DataFrame(parsed_data['clean_data'])
            raw_df = pd.DataFrame(parsed_data['raw_data'])

            print(f"오피스텔 매매 데이터 수집 완료: {len(clean_df)}개 항목 (정제된 데이터)")
            print(f"Raw 데이터: {len(raw_df)}개 항목")
            ...
            df = pd.DataFrame(parsed_data)
            print(f"오피스텔 매매 데이터 수집 완료: {len(df)}개 항목")
            return {
                'clean_data': df,
                'raw_data': None,
                'response_text': response_text
    ...
    def collect_data(self, data_type: str = "offi_rent", lawd_cd: str = None, deal_ymd: str = None) -> dict:
        """
        데이터 수집 (기존 호환성을 위한 래퍼 메서드)

        Args:
            data_type: 데이터 타입 ("offi_rent", "offi_trade")
            lawd_cd: 법정동 코드
            deal_ymd: 거래 년월
        ...
        if data_type == "offi_rent":
            return self.collect_offi_rent_data(lawd_cd, deal_ymd)
        elif data_type == "offi_trade":
            return self.collect_offi_trade_data(lawd_cd, deal_ymd)
        else:
            raise ValueError("data_type은 'offi_rent' 또는 'offi_trade'여야 합니다.")

    def display_summary(self, data_dict: dict, data_type: str = "offi_rent"):
        """
        수집된 데이터의 요약 정보 출력
