...
class RHDataCollector(BaseDataCollector):
    ...
    def _parse_line(self, line: str, api_type: str = "rh_rent") -> Optional[Dict]:
        """
        ...
        """
        try:
            house_info = line.split('2001')[0] if '2001' in line else line.split('2007')[0] if '2007' in line else line.split('2014')[0] if '2014' in line else line.split('2022')[0] if '2022' in line else line.split('2021')[0] if '2021' in line else line.split('2020')[0] if '2020' in line else line.split('2010')[0] if '2010' in line else line.split('2005')[0] if '2005' in line else line
    ...
    def collect_rh_rent_data(self, lawd_cd: str = None, deal_ymd: str = None) -> dict:
        """
    ...
    def collect_rh_trade_data(self, lawd_cd: str = None, deal_ymd: str = None) -> dict:
        """
    ...
    def collect_data(self, data_type: str = "rh_rent", lawd_cd: str = None, deal_ymd: str = None) -> dict:
        """
        데이터 수집 (기존 호환성을 위한 래퍼 메서드)

        Args:
            data_type: 데이터 타입 ("rh_rent", "rh_trade")
            lawd_cd: 법정동 코드
            deal_ymd: 거래 년월
        ...
        if data_type == "rh_rent":
            return self.collect_rh_rent_data(lawd_cd, deal_ymd)
        elif data_type == "rh_trade":
            return self.collect_rh_trade_data(lawd_cd, deal_ymd)
        else:
            raise ValueError("data_type은 'rh_rent' 또는 'rh_trade'여야 합니다.")

    def display_summary(self, data_dict: dict, data_type: str = "rh_rent"):
        """
        수집된 데이터의 요약 정보 출력
