"""
스케줄된 데이터 수집기 (collect_data_scheduled.py)
정기적으로 부동산 데이터를 수집하는 스케줄러 스크립트
"""
...
def main():
    """스케줄된 데이터 수집기 메인 함수"""
    parser = argparse.ArgumentParser(description='스케줄된 부동산 데이터 수집기')
    
    # 스케줄러 관련 옵션
    parser.add_argument('--schedule', action='store_true', help='스케줄러 모드 실행')
    parser.add_argument('--schedule_time', type=str, default='02:00', help='스케줄 실행 시간 (HH:MM)')
    ...
    parser.add_argument('--regions', nargs='+', default=get_lawd_codes(), 
                       help='법정동코드 리스트')
    parser.add_argument('--years', type=int, default=1, help='수집할 년수')
    parser.add_argument('--recent', action='store_true', help='최근 데이터만 수집')
    
    # 저장 옵션
    parser.add_argument('--no-save-to-vector-db', action='store_true', 
                       help='벡터 데이터베이스에 저장하지 않음')
    
    args = parser.parse_args()
    
    # 로거 설정
    logger = setup_logger('main', 'logs/collect.log')
    ...
    try:
        ...
        if args.recent:
            ...
            deal_ymds = get_recent_months(args.years * 12)
        
        logger.info(f"데이터 수집 시작: {data_types}, 지역: {args.regions}, 기간: {deal_ymds}")
        ...
        # 벡터 데이터베이스 저장
        if not args.no_save_to_vector_db and processed_results:
